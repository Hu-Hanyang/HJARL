...
class ConfigFactoryTestAdversary:
    '''Manager class that's in charge of experiment configs.'''
    ...
    def add_arguments(self):
        ...
        self.add_argument('--restore', type=str, help='folder to reload from')
        self.add_argument('--trained_task', type=str, help='task/environment')
        self.add_argument('--test_distb_level', default=0.0, type=float, help='disturbance level', metavar='')
        self.add_argument('--trained_distb_level', default=0.0, type=float, help='disturbance level', metavar='')
        self.add_argument('--render', action='store_true', help='make the video or gif')
    ...
    def merge(self, config_override=None):
        ...
        cmdline_dict = {k: v for k, v in args.__dict__.items() if v is not None}
        config_dict.update(cmdline_dict)
        # Hanyang: add the trained adversary model path
