...
class CartPoleTestEnv(BenchmarkEnv):
    '''Cartpole test disb basic environment task.
    ...
    def __init__(self,
        ...
        if init_state is None:
            ...
            raise ValueError('[ERROR] in CartPoleTestEnv.__init__(), init_state incorrect format.')
        ...
        if inertial_prop is None:
            ...
            raise ValueError('[ERROR] in CartPoleTestEnv.__init__(), inertial_prop incorrect format.')
    ...
    def reset(self, seed=None):
        ...
        if self.RANDOMIZED_INERTIAL_PROP:
            ...
            if any(phy_quantity < 0 for phy_quantity in prop_values.values()):
                raise ValueError('[ERROR] in CartPoleTestEnv.reset(), negative randomized inertial properties.')
    ...
    def _preprocess_control(self, action):
        ...
        # Apply disturbances.
        if 'action' in self.disturbances:  # CartPoleTestEnv(): self.disturbances = None now
    ...
    def _get_observation(self):
        ...
        if not np.array_equal(self.state, np.clip(self.state, self.state_space.low, self.state_space.high)) and self.VERBOSE:
            print('[WARNING]: observation was clipped in CartPoleTestEnv._get_observation().')
